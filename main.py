import os, sqlite3, logging, threading, time, secrets
...
def worker_menu():
    m=types.ReplyKeyboardMarkup(resize_keyboard=True,row_width=2)
    m.add("📦 Tovarlar","📊 Umumiy holat","🏪 Sklad")
    return m
...
def sklad_callback(call):
    ...
    # ── CHIQIM DETAIL (rasm + ma'lumot + tahrirlash + ayirish) ──
    ...
    # ── BARCHA MAHSULOTLAR (faqat nomlar) ──
...
function renderTable(){
  ...
  document.getElementById('prodBody').innerHTML=rows.map((p,i)=>{
    ...
    const badge=rem<=0?'<span class="badge paid">✅ To\'liq</span>':pct>0?'<span class="badge partial">⚠️ Qisman</span>':'<span class="badge unpaid">🔴 To\'lanmagan</span>';
